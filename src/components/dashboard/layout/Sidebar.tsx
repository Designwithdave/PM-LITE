  ...
  return (
    <div className="w-[280px] h-full bg-white/80 backdrop-blur-md border-r border-gray-200 flex flex-col">
        ...
        <div className="space-y-1.5">
          {items.map((item) => (
            <Button
              key={item.label}
              variant={"ghost"}
              className={`w-full justify-start gap-3 h-10 rounded-xl text-sm font-medium ${isActive(item.href) ? 'bg-blue-50 text-blue-600 hover:bg-blue-100' : 'text-gray-700 hover:bg-gray-100'}`}
              onClick={() => handleItemClick(item)}
            >
              <span className={`${isActive(item.href) ? 'text-blue-600' : 'text-gray-500'}`}>{item.icon}</span>
              {item.label}
            </Button>
          ))}
        </div>
      ...
      <div className="p-4 mt-auto border-t border-gray-200">
        ...
        >
          <span className="text-gray-500"><Settings size={20} /></span>
          Settings
        </Button>
        <Button
          variant="ghost"
          className="w-full justify-start gap-3 h-10 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-100 mb-1.5"
        >
          <span className="text-gray-500"><HelpCircle size={20} /></span>
          Help
        </Button>
      </div>
    </div>
